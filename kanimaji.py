...
def create_animation(filename):
    ...
    if SHOW_BRUSH:
        ...
        brush_brd_g = E.g(
            id="kvg:" + baseid + "-brush-brd-Kanimaji",
            style=(
                "fill:none;stroke:%s;stroke-width:%f;"
                + "stroke-linecap:round;stroke-linejoin:round;"
            )
    ...
    if GENERATE_GIF:
        ...
        for k in static_css:
            ...
            svgexport_data.append(
                {
                    "input": [abspath(svgframefile)],
                    "output": [[abspath(pngframefile), "%d:%d" % (GIF_SIZE, GIF_SIZE)]],
                }
            )
